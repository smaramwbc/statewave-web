...
// sitemap.xml is generated by scripts/generate-sitemap.mjs at build time
// (it depends on the MDX-compiled blog posts, which only Vite can resolve).
// Auto-skip if dist/ isn't built — the test runs after `npm run build`.
const distSitemapPath = resolve(repoRoot, 'dist', 'sitemap.xml')
const distBuildAvailable = existsSync(distSitemapPath)

describe.runIf(distBuildAvailable)('dist/sitemap.xml (generated)', () => {
  const xml = distBuildAvailable ? readFileSync(distSitemapPath, 'utf8') : ''
  ...
  it('lists at least one blog post URL', () => {
    expect(xml).toMatch(/<loc>https:\/\/www\.statewave\.ai\/blog\/[^<]+<\/loc>/)
  })
...
describe.runIf(!distBuildAvailable)('dist/sitemap.xml (generated)', () => {
  it('skipped — run `npm run build` first to exercise the sitemap generator', () => {
